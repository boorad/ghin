...
export class RequestClient {
  ...
  constructor(config: ClientConfig) {
    ...
    if (!results.success) {
      throw new ConfigurationError(
        `Invalid RequestClientConfig: ${results.error.message}`
      );
    }
  ...
  async fetch<RequestReturnType>({
    ...
    const accessTokenResult = await this.lock.runExclusive(async () =>
      this.getAccessToken()
    );
    ...
    return withRetry(() =>
      this._fetch<RequestReturnType>({ options: actualOptions, schema, url })
    );
  }
}
