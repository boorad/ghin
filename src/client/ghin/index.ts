...
export class GhinClient {
  ...
  courses: {
    ...
    ) => Promise<CourseSearchResponse["courses"]>;
  };
  ...
  golfers: {
    getOne: (
      ghinNumber: number
    ) => Promise<GolfersSearchResponse["golfers"][number] | undefined>;
    getScores: (
      ghinNumber: number,
    ...
    ) => Promise<GolfersSearchResponse["golfers"]>;
    globalSearch: (
  ...
  constructor(config: ClientConfig) {
    ...
    if (!results.success) {
      throw new ConfigurationError(
        `Invalid GhinClientConfig: ${results.error.message}`
      );
    }
  ...
  private async courseGetDetails(
    request: CourseDetailsRequest
  ): Promise<CourseDetailsResponse> {
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new ValidationError(
          `Invalid course details request: ${error.message}`
        );
      }
      throw error instanceof Error ? error : new Error(String(error));
    }
  ...
  private async courseSearch(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new ValidationError(
          `Invalid course search request: ${error.message}`
        );
      }
      throw error instanceof Error ? error : new Error(String(error));
    }
  ...
  private async facilitySearch(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new ValidationError(
          `Invalid facility search request: ${error.message}`
        );
      }
      throw error instanceof Error ? error : new Error(String(error));
    }
  ...
  private async handicapsGetCoursePlayerHandicaps(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new ValidationError(
          `Invalid course handicap request: ${error.message}`
        );
      }
      throw error instanceof Error ? error : new Error(String(error));
    }
  ...
  private async golfersSearch(
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new ValidationError(
          `Invalid golfer search request: ${error.message}`
        );
      }
      throw error instanceof Error ? error : new Error(String(error));
    }
