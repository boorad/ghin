...
- [ ] Phase 1 — schema fix: `z.custom<CacheClient>` in `src/models/cache-client.ts`, explicit `CacheClient` type, drop dead helper schemas, rewrite/add tests in `cache-client.test.ts` (identity: `parse(x) === x` for plain object and class instance) and `client-config.test.ts`
