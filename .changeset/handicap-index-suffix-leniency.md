...
GHIN returns an index like `"19.1M"` (modified by the Handicap Committee) or `"12.4WD"` (withdrawn) for golfers under a WHS status. `handicap_index` was declared as `float` (`z.coerce.number()`), which turned those into `NaN` and failed the parse — the same production bug fixed for `golfers.search` in #56. Because these responses are plain arrays, one such golfer fails the **entire batch**: a request for course or playing handicaps for a foursome with a single `M`/`WD` player returns nothing for anyone. (Observed in production on `golfers.search`; on these two endpoints the failure follows from the same mechanism rather than from a captured payload.)
